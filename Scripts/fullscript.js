...
function mainFunction() {
	//Determines exact date
		try {
			if (percentLifePassed < 0) {
				text = invalidDOB;
			}
			else if (realYearsAlive > 100) {
				text = aboveLE
			}
		...
		if ((currentDay - dob) < 0) {
			...
			writeDays2 = Math.abs(currentDay - dob);
		}
		else {
			writeDays2 = Math.abs(currentDay - dob)
		}
	...
	//If '1', no 's' on the end of years/months/days/hours/minutes
		...
		if ((currentDay - dob) == 1) {
			...
			writeDays2 = "and " + Math.abs(currentDay - dob) + " days old";
		}
	...
	//Determines the 'more infomation' section of the page
		...
		if ((isNaN(gender + countryValue + realYearsAlive)) || (text == invalidDOB) || (gender == "none")) {
			moreInfo = "";
		}
